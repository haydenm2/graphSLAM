...
class Robot():
    ...
    def linearize(self):
        ...
        self.info_matrix[0, 0] = 1e7
        self.info_matrix[1, 1] = 1e7
        self.info_matrix[2, 2] = 1e7

        self.linearize_controls()
        self.linearize_measurements()

    def reduce(self):
        self.info_matrix_tilde = np.copy(self.info_matrix)
        self.info_vec_tilde = np.copy(self.info_vec)
    ...
    def solve(self):
        try:
            self.cov_final = np.linalg.inv(self.info_matrix_tilde[0:3 * self.num_states, 0:3 * self.num_states])
        except np.linalg.LinAlgError:
            self.cov_final = np.linalg.pinv(self.info_matrix_tilde[0:3 * self.num_states, 0:3 * self.num_states])
        self.mu_final = self.cov_final @ self.info_vec_tilde[0:3 * self.num_states].reshape(
            (len(self.info_vec_tilde[0:3 * self.num_states]), 1))

        # ---------------- ASSUMING ALL LANDMARKS VISIBLE ALWAYS --------------------------
        for j in range(self.num_lm):
            lm_idx = (3 * self.num_states) + (2 * j)

            mat_jj = self.info_matrix[lm_idx:lm_idx + 2, lm_idx:lm_idx + 2]
            vec_j = self.info_vec[lm_idx:lm_idx + 2].reshape((2, 1))
            mat_jT = self.info_matrix[lm_idx:lm_idx + 2, 0:3 * self.num_states]
            mu_T = self.mu_final

            self.mu_lm_final = np.linalg.inv(mat_jj) @ (vec_j + mat_jT @ mu_T)
    ...
    def calculate_cost(self):
        ...
        for i in range(self.num_states):
            xt_error = np.array([[1], [1], [1]])  # x - g(u,x_(t-1))    # TODO -------------------
            try:
                R_inv = np.linalg.inv(self.get_R(i))
            except np.linalg.LinAlgError:
                R_inv = np.linalg.pinv(self.get_R(i))
            x_error_term += xt_error.transpose() @ R_inv @ xt_error

            for j in range(self.num_lm):
                zt_error = np.array([[1], [1]])  # z - h(y,c)    # TODO -------------------
                x_error_term += zt_error.transpose() @ np.linalg.inv(self.Q_t) @ zt_error

        self.J = x0.transpose() @ self.info_matrix[0:3, 0:3] @ x0 + x_error_term + z_error_term
